...
export async function createPurchaseRequest(prevState: ActionState | null, formData: FormData): Promise<ActionState> {
  ...
  const presentation = formData.get("presentation")?.toString() || "";
  ...
  try {
    ...
    await connection.query(
        `UPDATE purchase_requests 
         SET product_code = ?, category = ?, product_type = ?, presentation = ? 
         WHERE id = ?`,
        [product_code, category, product_type, presentation, newRequestId]
    );
  ...
  try {
    ...
    await connection.query(
        `UPDATE purchase_requests 
         SET product_code = ?, category = ?, product_type = ?, presentation = ? 
         WHERE id = ?`,
        [product_code, category, product_type, presentation, requestId]
    );
...
export async function createConsolidatedPurchaseRequest(prevState: ActionState | null, formData: FormData): Promise<ActionState> {
  const session = await auth();
  if (!session?.user?.id) return { success: false, message: "Sesión expirada" };
  const userId = session.user.id;
  ...
  if (!branch_id || !items_json) return { success: false, message: "No hay productos para procesar" };
  ...
  try {
    ...
    const savedFiles: { name: string, publicUrl: string }[] = [];
    if (files && files.length > 0) {
        const uploadDir = join(process.cwd(), "public/uploads");
        try { await mkdir(uploadDir, { recursive: true }); } catch (err) {}
        for (const file of files) {
            if (file.size > 0) {
                ...
                const filePath = join(uploadDir, fileName);
                await writeFile(filePath, Buffer.from(bytes));
                savedFiles.push({ name: file.name, publicUrl: `/uploads/${fileName}` });
            }
        }
    }
    ...
    for (const item of items) {
        try {
            const [rows]: any = await pool.query(
                "CALL sp_crear_solicitud(?, ?, ?, ?, ?, ?)",
                [branch_id, userId, issue_date, item.nombre, item.total, currency]
            );
            const newRequestId = rows[0][0]?.id;
            ...
            if (newRequestId) {
                await pool.query(
                    `UPDATE purchase_requests 
                     SET product_code = ?, category = ?, product_type = ?, presentation = ? 
                     WHERE id = ?`,
                    [item.codigoProd, item.categoria, item.tipo, item.unidad, newRequestId]
                );

                await pool.query(
                    `INSERT INTO purchase_request_details (request_id, product_name, quantity, unit_measure, estimated_price) 
                     VALUES (?, ?, ?, ?, ?)`,
                    [newRequestId, item.nombre, item.cantidad, item.unidad, item.precio]
                );

                for (const f of savedFiles) {
                    await pool.query("CALL sp_insertar_cotizacion(?, ?, ?)", [newRequestId, f.name, f.publicUrl]);
                }
                totalGeneradas++;
            }
        } catch (itemError) {
            console.error(`Error al crear la solicitud para ${item.nombre}:`, itemError);
            errores++; 
        }
    }
    ...
    if (errores > 0) {
        return { success: true, message: `Terminado: ${totalGeneradas} solicitudes creadas. Hubo ${errores} errores.` };
    }
    return { success: true, message: `¡Éxito! Se han generado ${totalGeneradas} solicitudes con sus categorías.` };
  } catch (error: any) {
    return { success: false, message: "Error crítico al procesar el Excel: " + error.message };
  }
}
    ...
    try {
        ...
        if (role !== 'GERENTE GENERAL') {
            ...
            if (montoTotal > limitePermitido) {
                await connection.rollback();
                return { success: false, message: `Permiso denegado. El límite operativo es S/ ${limitePermitido}, la solicitud de S/ ${montoTotal} requiere aprobación superior.` };
            }
        }
...
export async function obtenerReporteCompras(branchId: number, fechaInicio: string, fechaFin: string) {
    ...
    try {
        ...
        console.error("🚨 ERROR SQL EN REPORTE:", error.message);
...
export async function obtenerPrediccionCompras(branchId: number) {
    ...
    try {
        ...
        console.error("🚨 ERROR SQL EN PREDICCIÓN:", error.message);
        return { success: false, data: [] }; 
    } finally { 
        connection.release(); 
    }
